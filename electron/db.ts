...
export class DBManager {
  ...
  constructor() {
    const dbPath = path.join(app.getPath('userData'), 'hardware-pos.db');
  ...
  private initSchema() {
    ...
    migrateTable('sales', [
      ...
      { name: 'customer_name', type: 'TEXT', default: "'Walk-in Customer'" },
      { name: 'cashier_id', type: 'TEXT', default: "''" }
    ]);
    ...
    if (userCount.count === 0) {
      ...
      // Seed default store name for hardware store
      this.db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)').run('storeName', 'My Hardware Store');
    }
  }
  ...
  public updateProduct(product: any, variants: any[]) {
    ...
    const updateVariant = this.db.prepare(`
      UPDATE variants 
      SET size = @size, color = @color, stock_qty = @stock_qty, cost_price = @cost_price, selling_price = @selling_price
      WHERE id = @id
    `);
  ...
  public processSale(saleData: any) {
    console.log('Processing sale:', JSON.stringify(saleData, null, 2));
    ...
    const insertSale = this.db.prepare(`
      INSERT INTO sales (receipt_number, total, payment_method, customer_name, cashier_id)
      VALUES (@receipt_number, @total, @payment_method, @customer_name, @cashier_id)
    `);
    ...
    const transaction = this.db.transaction(() => {
      const saleResult = insertSale.run({
        ...
        cashier_id: saleData.cashier_id || 'admin'
      });
