...
export const Sales: React.FC = () => {
  ...
  const addToCart = (product: ProductWithVariants, variantId: number, size: string, color: string, price: number, cost: number) => {
    setCart(prev => {
    ...
    <div className="flex h-full">
                    ...
                    <tr 
                      ...
                      onClick={() => {
                        ...
                        if (product.variants.length === 1 && 
                            product.variants[0].size === 'Each' && 
                            product.variants[0].color === 'Generic') {
                          addToCart(product, product.variants[0].id, 'Each', 'Generic', product.variants[0].selling_price > 0 ? product.variants[0].selling_price : product.selling_price, product.variants[0].cost_price > 0 ? product.variants[0].cost_price : product.cost_price);
                        } else {
      ...
      {selectedProduct && (
        <VariantSelectorModal 
          ...
          onSelect={(variantId, size, color, price, cost) => addToCart(selectedProduct, variantId, size, color, price, cost)}
        />
      )}
