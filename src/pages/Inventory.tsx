  ...
  const handleExport = () => {
    const dataToExport = filteredProducts.map(p => {
      ...
      return {
        ...
        'Variants': p.variants.map(v => (v.size === 'Each' && v.color === 'Generic') ? `Default (${v.stock_qty})` : `${v.size} · ${v.color} (${v.stock_qty})`).join(', ')
      };
    });
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {product.variants.map(v => (
                          <span key={v.id} className="text-xs bg-gray-100 px-2 py-1 rounded border">
                            {(v.size === 'Each' && v.color === 'Generic') ? 'Default' : `${v.size} · ${v.color}`} ({v.stock_qty})
                          </span>
                        ))}
                      </div>
                    </td>
